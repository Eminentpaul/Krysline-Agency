...
class InvestmentPlan(models.Model):
    ...
    name = models.CharField(max_length=20, choices=PlanType.choices, unique=True)
    ...
    def calculate_total_return(self, investment_amount):
        ...
        if self.is_annual_roi:
            # Annual ROI compounded or simple?
            years = self.duration_months / 12
            return investment_amount * (self.roi_percentage / 100) * years
        else:
            # Simple ROI for the period
            return investment_amount * (self.roi_percentage / 100)
...
class Investment(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == InvestmentStatus.ACTIVE and not self.start_date:
            ...
            self.total_expected_return = self.plan.calculate_total_return(self.amount)
    ...
    def next_payout_date(self):
        ...
        next_payout = self.start_date + timezone.timedelta(days=months_passed * 30)
        return next_payout
